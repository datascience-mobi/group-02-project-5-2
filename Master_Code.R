...
PatientInfo <- annotation[,c(3,4,11,29,34,35,36,37)]
...
Beta_Results <- beta_both_clean[rownames(Ttest_results),-3]
Beta_Results <- cbind(Beta_Results, AML_Mean = log2(rowMeans(Beta_Results[,1:8])), CON_Mean = log2(rowMeans(Beta_Results[,9:17])))
Beta_Results <- cbind(Beta_Results, Mean_diff = Beta_Results$AML_Mean - Beta_Results$CON_Mean)

plot(Beta_Results$Mean_diff, -log10(Ttest_results$pval), main = "Volcano Plot: T-test Results",  xlab = "Fold change", ylab = "-log10(p.value)")

Beta_Results_001 <- Beta_Results[rownames(Ttest_results_rankedPValue_001),]
Beta_Results_005 <- Beta_Results[rownames(Ttest_results_rankedPValue_005),]
Beta_Results_010 <- Beta_Results[rownames(Ttest_results_rankedPValue_01),]

points( Beta_Results_010[,20], -log10(Ttest_results[rownames(Beta_Results_010),2]), col = "red")
points( Beta_Results_005[,20], -log10(Ttest_results[rownames(Beta_Results_005),2]), col = "orange")
points( Beta_Results_001[,20], -log10(Ttest_results[rownames(Beta_Results_001),2]), col = "blue")
